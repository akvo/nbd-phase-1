  ...
  return {
    Webform: ({ forms, onFinish }: any) => (
      <div data-testid="mock-webform">
        ...
        <button data-testid="submit-btn" onClick={() => onFinish({ '1': 'GOOD', 'wetland_id': 1 })}>
          Submit Form
        </button>
      </div>
    ),
...
describe('NewFormPage', () => {
  test('renders form blueprint when loaded successfully', async () => {
    ...
    // Shows loading first
    expect(screen.getByText(/Fetching form blueprint definition/i)).toBeInTheDocument();
    ...
    expect(screen.getByText('Form: Focus Group Discussion')).toBeInTheDocument();
  });
