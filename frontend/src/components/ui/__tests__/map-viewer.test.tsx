...
vi.mock('react-leaflet', () => {
  return {
    ...
    GeoJSON: ({ data }: any) => <div data-testid="geojson" data-geojson={JSON.stringify(data)} />,
    useMap: () => ({
      fitBounds: vi.fn(),
    }),
  };
...
test('renders basin geometry overlay when provided', () => {
  const center: [number, number] = [0, 0];
  const mockGeoJson = { type: 'Feature', geometry: { type: 'MultiPolygon', coordinates: [] } };
  const { getByTestId } = render(<MapViewer center={center} zoom={13} basinGeometry={mockGeoJson} />);
