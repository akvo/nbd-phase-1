...
Currently, the frontend `frontend/src/components/ui/site-drawer.tsx` hardcodes **Turbidity** (`38 NTU`) and the **Macroinvertebrate** index (`0.48 index`) in the raw sampling method table because the `sampling_records` table does not contain fields for them.
