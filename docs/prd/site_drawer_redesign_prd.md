...
3. The header section displays the site name, code, a solid status color circle badge (representing health class severity), country pin, last reported timestamp, and adjust state.
4. A warning alert notice is displayed if the site's health is critical or at-risk.
5. Below this, the user sees the structured grids:
...
1. **Header Layout**: Left side shows name and ID; right side shows a solid circular status color badge representing the health class severity (green for healthy, orange for at risk, red for critical).
2. **Horizontal Status Badges**: Country badge with map-pin icon, Last Reported timestamp badge, and dynamic `IK-adjusted` badge (displayed if applicable).
...
- Badges: `site.country`, `site.last_updated`, `site.is_ik_adjusted`
