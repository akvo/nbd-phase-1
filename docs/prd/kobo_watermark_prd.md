...
### UAC 3A.1 (Interval Pull & Sync)

- Given the background worker wakes up at the configured synchronization interval (defaulting to 60 minutes).
...
- **FR-3A.1 (Configurable Interval Trigger)**: The APScheduler worker wakes up at a configurable interval in minutes defined by the `KOBO_SYNC_INTERVAL_MINUTES` environment variable (default: 60 minutes).
